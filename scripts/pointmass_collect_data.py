...
def collect(num, seed, epsilon, frame_stack, maze_spec, idx, queue):
    env = FrameStack(VisualMazeEnv(maze_spec=maze_spec), frame_stack)
    env.seed(seed + idx)
...
def main():
    ...
    with h5py.File(
        f"pointmass_dataset_{args.maze_spec}_s{args.seed}_e{args.epsilon}_size{args.size}.hdf5", "w"
    ) as f:
        for k, v in data.items():
            f.create_dataset(k, data=v, compression="lzf")

    with h5py.File(
        f"pointmass_dataset_{args.maze_spec}_s{args.seed}_e{args.epsilon}_size{args.size}.hdf5", "r"
    ) as f:
